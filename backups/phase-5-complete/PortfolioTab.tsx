...
export default function PortfolioTab() {
  ...
  useEffect(() => {
    ...
    if (selection.selectedContent?.id !== selectedContent?.id ||
      ...
      // Ensure pageState is expanded-reader (already set in handleCollectionClick, but ensure it)
  ...
  // Collection tab opening: Add to activeCollections, switch activeTab, set expanded-reader state
  const handleCollectionClick = useCallback((collection: Collection) => {
    ...
    setActiveTab(collection.slug)
    // Set pageState to expanded-reader per logic doc line 103 (collection tab loads in this state)
    setPageState('expanded-reader')
  }, [activeCollections])
  ...
  return (
    <div>
      {/* Profile component at top - sticky header reports its height via callback */}
      <Profile onHeightChange={setProfileHeight} />
      
      {/* PortfolioTab main content */}
      <div className="flex-1 py-3 px-8 pb-6 relative">
        {/* Content area wrapper with max-width constraint */}
        <div className="max-w-[1280px] mx-auto w-full h-full flex justify-between items-start relative">
          {/* Left sidebar container - sticky positioning aligned to Profile bottom */}
          <div 
            className="flex-shrink-0 flex gap-[3rem] items-start z-10 p-4 sticky self-start"
            style={{ top: profileHeight }}
          >
            <MainMenu
          ...
          {/* ContentReader and InfoMenu - only visible in reader states (collapsed-reader, expanded-reader) */}
          ...
          {/* Collections Menu - right side (Step 5.1 Stage 5) */}
          <div 
            className="flex-shrink-0 z-10 p-4 sticky self-start"
            style={{ top: profileHeight }}
          >
            <CollectionsMenu
      ...
      {/* BottomTabBar component - fixed positioning at bottom */}
      <BottomTabBar
        activeTab={activeTab}
