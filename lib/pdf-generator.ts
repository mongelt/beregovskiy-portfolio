...
/**
 * Entry point. Handles BlockNote format (array) and legacy EditorJS format ({ blocks: [...] }).
 */
function renderBlockNoteContent(data: any): string {
  if (!data) return '<p>No content available</p>'

  // Legacy EditorJS format: { blocks: [...] }
  if (!Array.isArray(data) && Array.isArray(data.blocks)) {
    return renderEditorJSContent(data)
  }

  // BlockNote format: array of block objects
...
function renderBNBlock(block: any): string {
  ...
  switch (block.type) {
    ...
    case 'button': {
      ...
      const bg = block.props?.buttonBackgroundColor || '#2563eb'
      const fg = block.props?.buttonTextColor || '#ffffff'
      ...
      const btnStyle = `display: inline-block; width: ${width}px; padding: 8px 14px; background: ${escapeHtml(bg)}; color: ${escapeHtml(fg)}; text-align: center; font-weight: 500; text-decoration: none;`
